...
DEFAULT_MEDIA_SEARCH_PATHS = [
    r"D:\1-6 AI_Agent\2_local_project\EasyScene\backend\47d8edbd-5299-4917-be75-1f36396b156d"
...
class MediaResolver:
    ...
    def _make_result(
        self, found: bool, path: Optional[str], query: str, candidates: list[str] = None
    ) -> dict:
        result = {
            "found": found,
            "path": path,
...
def create_media_resolver_tool(
    search_paths: list[str] = None, extra_paths: list[str] = None, base_dir: str = None
):
    """创建媒体解析工具"""
    ...
    def resolve_media(query: str) -> str:
        ...
        return (
            f"未找到匹配 '{query}' 的文件。请检查文件名或使用 list_media 查看可用文件。"
        )
