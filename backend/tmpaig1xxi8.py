...
project = JyProject("缉凶者拼接项目", width=1920, height=1080)
...
# 获取第一个视频时长（微秒）
dur1 = project.get_track_duration("main")
print(f"视频1时长: {dur1} 微秒")

# 添加第二个视频，从第一个视频结束处开始
project.add_media_safe(video2, str(dur1))

# 添加模糊转场 - 在两个视频之间
# 转场开始时间 = 第一个视频结束前 0.5 秒
trans_dur = 500000  # 0.5秒 = 500000微秒
trans_start = dur1 - trans_dur
print(f"转场开始时间: {trans_start}")

try:
    project.add_transition_simple("blur", trans_start, trans_dur)
    print("转场添加成功")
