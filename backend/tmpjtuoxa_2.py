...
# 查看 script 属性
print("script attrs:", [a for a in dir(project.script) if not a.startswith('__')])
print()

# 查看 tracks
if hasattr(project.script, 'tracks'):
    print("tracks:", project.script.tracks)
    for t in project.script.tracks:
        print(f"  Track type: {t.type}, segments: {len(t.segments)}")