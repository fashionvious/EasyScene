...
for track in tracks:
    print(f"Track: {track.type} - {track.name}")
    for segment in track.segments:
        print(f"  Segment: {segment.id}, material_type={segment.material_type}")
        # 检查是否有 TTS 相关的 material
        if hasattr(segment, 'material_id'):
            print(f"    material_id: {segment.material_id}")